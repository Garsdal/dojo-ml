...
- **Task contract scaffolded, not yet enforced** — [core/task.py](src/dojo/core/task.py) defines `Task` + `TaskType.REGRESSION` + `TASK_TYPE_REGISTRY`; `Domain` has `task: Task | None` and `program_path: str | None` fields; [runtime/task_service.py](src/dojo/runtime/task_service.py) handles create/freeze/unfreeze with `TaskNotReadyError` / `TaskFrozenError`; HTTP routes live under `/domains/{id}/task`. The orchestrator does *not* yet enforce `task.frozen` at run start — that's Phase 3 work in [NEXT_STEPS.md](NEXT_STEPS.md).
- **`Domain.tools` still exists alongside `domain.task.tools`** — Phase 1 deliberately kept the old field for backward compatibility. The migration to "tools live only on Task" happens in Phase 3 alongside the anti-cheating wiring.
