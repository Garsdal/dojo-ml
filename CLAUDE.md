...
- **Regression contracts (current shape, version 2)** — `def evaluate(y_pred, *, X_train, X_test, y_train, y_test) -> dict` and `def train(X_train, y_train, X_test) -> y_pred`. Agent code does NOT call `load_data` from inside either tool — the framework loads data once and passes splits as parameters.
