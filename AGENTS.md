...
YAML at `.agentml/config.yaml`, overridable via env vars.
...
| `agent` | `backend`, `max_turns`, `max_budget_usd`, `permission_mode`, `cwd` | `claude`, `50` |

### Environment Variable Naming

**⚠️ Gotcha:** The pydantic-settings prefix is `AGENTML_` (single trailing underscore), and nested fields are separated by `__` (double underscore). This means an env var has **one** `_` between `AGENTML` and the top-level field, not two:

```
AGENTML_<FIELD>__<SUBFIELD>=value
        ^      ^^
        |      |└─ nested delimiter (double underscore)
        |      └── nested delimiter (double underscore)
        └── prefix already ends with _ (single underscore)
```

| Correct ✅ | Wrong ❌ (silently ignored) |
|---|---|
| `AGENTML_AGENT__BACKEND=stub` | `AGENTML__AGENT__BACKEND=stub` |
| `AGENTML_TRACKING__BACKEND=mlflow` | `AGENTML__TRACKING__BACKEND=mlflow` |
| `AGENTML_TRACKING__MLFLOW_TRACKING_URI=...` | `AGENTML__TRACKING__MLFLOW_TRACKING_URI=...` |

Pydantic-settings will **silently ignore** misspelled env vars — defaults kick in with no warning. If overrides don't seem to work, double-check the underscore count.
